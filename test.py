...
parser.add_argument('--rgb_files',  default='./data/rgb/*.jpg', help='folder name of rgb image')
parser.add_argument('--depth_files',  default='./data/depth_mm/*.png', help='folder name of low resolution depth image')
parser.add_argument('--lr_files',  default='./data/lr_depth/', help='folder name of low resolution depth image')
parser.add_argument('--scale', type=int, default=4, help='scale factor')
# parser.add_argument('--parameter',  default='./data/parameter/parameter57', help='name of parameter file')
parser.add_argument('--parameter',  default='./data/parameter/parameter120', help='name of parameter file')
...
# test_files_down = glob.glob('./data/nyu_depths/test/*.png')
...
for i in range (len(test_files_down)):
    ...
    # lr = np.array(Image.fromarray(lr).resize((320,240),Image.BICUBIC).resize((640,480), Image.BICUBIC))
    cv2.imwrite(opt.lr_files  + test_files_down[i][16:-6]+'LR.png', lr)
    # lr_lat = lr
    # print(lr_pre == lr_lat)
    # print(lr)
    ...
    output = (res_img[0, 0]*(maxx-minn)+minn).astype(np.float32)
    output2 = output
    # print(output2)
    tx = time.time()
    temp = output > 60000
    ...
    print("t",ty-tx)
    cv2.imwrite(opt.output_folder  + test_files_down[i][16:-6]+'HR.png', output)# please modify '36' 
    t6 = time.time()
...
print("RMSE:"+str(rmse/len(output)))
